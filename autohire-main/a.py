...
def main():
    ...
    for i, resume in enumerate(resumes):
        ...
        # Extract skill names (ensure name exists and is a string)
        resume_skills = [str(skill.get('name')) for skill in skills_list if skill.get('name')]
        ...
        row = {
            ...
            'resumeSkills': resume_skills, # List of strings
    ...
    try:
        for target, model in trained_models.items():
            if target in ['skillsScore', 'experienceScore', 'locationScore', 'roleSimilarity', 'educationScore', 'matchScore']:
                 # Ensure the model's expected features match the processed data columns
                 # Note: Scikit-learn models trained directly might not have feature_names_in_ if not trained on DataFrames directly
                 # It's safer to rely on the feature_preserver.feature_cols for alignment
                 if list(X_processed.columns) != feature_preserver.feature_cols:
                     ...
                     # Consider raising an error here if strict matching is required
                 eprint(f"Predicting for target: {target}") # Debug print
                 predictions[target] = model.predict(X_processed)
            else:
                 eprint(f"Skipping prediction for unexpected target in loaded model: {target}")

        prediction_df = pd.DataFrame(predictions, index=X_processed.index)

        # Add resume IDs back using the original DataFrame's index alignment
        prediction_df['_id'] = input_df.loc[X_processed.index, '_id'].values

        # Add match category based on matchScore thresholds for easier frontend use
        ...
        output_data = prediction_df.astype({
             col: float for col in prediction_df.columns if col not in ['_id', 'match_category'] # Float scores
         }).astype({
             'match_category': int # Int category
